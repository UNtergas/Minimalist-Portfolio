...
export default function ProjectPage() {
    const [projects, setProjects] = useState<MarkdownData<ProjectFrontMatter>[]>([]);
    const [sandboxs, setSandboxs] = useState<MarkdownData<ProjectFrontMatter>[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            const projectsData = await getMarkdownData<ProjectFrontMatter>("project").catch(() => []);
            const sandboxData = await getMarkdownData<ProjectFrontMatter>("sandbox").catch(() => []);
            setProjects(projectsData);
            setSandboxs(sandboxData);
        };

        fetchData();
    }, []);

    //Apply GSAP animations with timeline
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            ...
            // Animate large projects
            projectRefs.current.forEach((el, index) => {
                const animationProps = index % 2 === 0 // Even-indexed projects (move from left)
                    ? { x: -100, opacity: 0.5 }
                    : { x: 100, opacity: 0.5 }; // Odd-indexed projects (move from right)
